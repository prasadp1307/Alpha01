    ...
    return (
        <nav className="h-16 glass-card border-b border-white/5 flex items-center justify-between px-8 sticky top-0 z-50">
            ...
            <div className="flex items-center gap-6">
                <div className="flex items-center gap-3 bg-slate-900/60 hover:bg-slate-800/60 px-4 py-2 rounded-2xl border border-white/5 transition-all text-sm group">
                    <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.5)]" />
                    <div className="flex flex-col">
                        <span className="font-bold text-slate-100 leading-none">{user?.name}</span>
                        <span className="text-[10px] text-slate-500 font-medium uppercase tracking-widest mt-1 group-hover:text-primary-400 transition-colors">{user?.role}</span>
                    </div>
                </div>
                <button 
                    onClick={logoutUser}
