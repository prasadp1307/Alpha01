    ...
    return (
        <div className="animate-fade-in">
            <header className="mb-10">
                <h2 className="text-3xl font-black text-white tracking-tight">System <span className="text-primary-500">Overview</span></h2>
                <p className="text-slate-500 mt-2">Manage your workforce activities and metrics in real-time.</p>
            </header>
                ...
                {cards.map((card) => (
                    <div key={card.label} className="glass-card group hover:scale-[1.02] active:scale-[0.98]">
                        <div className="p-6">
                            ...
                            <h4 className="text-4xl font-extrabold text-white mb-1">{card.value}</h4>
                            <p className="text-sm font-semibold text-slate-500 tracking-wide uppercase">{card.label}</p>
                        </div>
            ...
            <div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="flex flex-col gap-4">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5">
                                <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-primary-400 font-bold">JD</div>
                                <div className="flex-1">
                                    <p className="text-sm font-bold text-white">John Doe marked attendance</p>
                                    <p className="text-xs text-slate-500">2 hours ago • Product Design</p>
                                </div>
                                <span className="text-[10px] font-bold text-green-500 uppercase tracking-widest bg-green-500/10 px-2 py-1 rounded">Success</span>
                            </div>
                        ))}
                    </div>
                </div>
                <div className="glass-card p-8 bg-gradient-to-br from-primary-600/10 to-transparent">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {['Register Employee', 'Post Task', 'Review Leaves', 'Audit Logs'].map(link => (
                            <button key={link} className="flex items-center justify-between p-4 rounded-xl border border-white/5 bg-slate-900/40 hover:bg-primary-600/20 hover:border-primary-500/30 transition-all text-slate-300 hover:text-white font-medium group text-sm">
                                {link}
